...
class GrocersAdap (private val context: Context, private val Grocslist: ArrayList<Grocers>, private var firestore: FirebaseFirestore, private var auth: FirebaseAuth) : RecyclerView.Adapter<GrocersAdap.MyGrocsView>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyGrocsView {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.itemintabs, parent, false)
        return MyGrocsView(itemView)
    }

    override fun onBindViewHolder(holder: MyGrocsView, position: Int) {
        ...
        val currentitem = Grocslist[position]
        var totalquantity = 1
        var totalprice = 0
        ...
        holder.mstocks.text = currentitem.stock
        Glide.with(context).load(currentitem.image).placeholder(circularProgressDrawable)
            .into(holder.mimage)
        totalprice = currentitem.cost * totalquantity

        if (holder.mstocks.text.toString().contains("Unavailable")){
            holder.addbtn.visibility = View.GONE
            holder.mstocks.visibility =View.VISIBLE
...
//            totalprice = intent?.getIntExtra("cost",0)!! * totalquantity
    ...
    override fun getItemCount(): Int {
        return Grocslist.size
    }

    class MyGrocsView(itemView : View): RecyclerView.ViewHolder(itemView){
        ...
        var mstocks : TextView = itemView.findViewById(R.id.stock)
